...
var startTime = null; // Variable to store start time
var isRunning = false; // Flag to indicate if timer is running
...
function ButtonPress1() {
  ...
  // Toggle timer and test duration display
  if (!isRunning) {
    startTime = new Date(); // Capture start time on first press
    isRunning = true;
    startDataCollection();
    document.getElementById("test-duration").textContent = "Test Duration: 0 mins, 0 sec";
  
    // Record experiment start time in CSV string (Column A, Column E)
    csvData += `\n${startTime.toLocaleTimeString('en-US')}, , , , experiment trial started`;
  
  } else {
    isRunning = false; // Pause timer on second press
  }
}

// Update test duration every second
setInterval(function() {
  if (isRunning) {
    var currentTime = new Date();
    var elapsedTime = currentTime - startTime; // Time difference in milliseconds

    // Convert milliseconds to minutes and seconds
    var minutes = Math.floor(elapsedTime / (1000 * 60));
    var seconds = Math.floor((elapsedTime % (1000 * 60)) / 1000);

    // Format and display test duration
    document.getElementById("test-duration").textContent = "Test Duration: " + minutes + " mins, " + seconds + " sec";
  }
}, 1000); // Update every 1 second


  function ButtonPress1_old() {
    var xhttp = new XMLHttpRequest();
    xhttp.open("PUT", "BUTTON_RUN", false);
    xhttp.send();
  ...
  // Function to handle response and update plot (called every 1 second)
  var plotUpdateTimer = null;
  ...
  function response() {
    ...
    if (msg == 0) { // not running
      ...
      // Record experiment start time in CSV string (Column A, Column E)
      csvData += `\n${startTime.toLocaleTimeString('en-US')}, , , , experiment trial ended`;
