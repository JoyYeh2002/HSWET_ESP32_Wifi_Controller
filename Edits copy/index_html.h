...
function downloadURI(uri, name) {
  ...
  function ButtonPress1() {
    ...
    function SetOperationMode(mode_str) {
      ...
      radioMsg.textContent = `HEY WE ENTERED THE CONTENT "${mode_str}".`; // Update console message here
      var xhttp = new XMLHttpRequest();
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          ...
          radioMsg.textContent = `OH YEA set to "${mode_str}".`; // Update console message here
        
        }
      }
    
      if (mode_str == "PWR OPTI") {
        xhttp.open("PUT", "PWR_OPTI", true); // Use asynchronous request
        xhttp.send();
        radioMsg.textContent = `OPTII "${mode_str}".`; // Update console message here
    } else if (mode_str == "DURABILITY") {
        xhttp.open("PUT", "DURABILITY", true); // Use asynchronous request
        xhttp.send();
        radioMsg.textContent = `DURA "${mode_str}".`; // Update console message here
    } else if (mode_str == "RATED PWR") {
        xhttp.open("PUT", "RATED_PWR", true); // Use asynchronous request
        xhttp.send();
        radioMsg.textContent = `RATED "${mode_str}".`; // Update console message here
    } else {
        radioMsg.textContent = 'ERROR: Invalid mode selected.'; // Update console message here
        return;
    }
    }
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 radioButtons.forEach(radio => {
  ...
  sendButton.addEventListener('click', function() {
      ...
      if (selectedRadio) {
          const selectedLabel = selectedRadio.nextElementSibling.textContent;
          // Actually send the selected mode to the ESP32 here
